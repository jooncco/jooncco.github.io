...
```cpp
unsigned int dough = 0;                  // 0000 0000 0000 0000 0000 0000 0000 0000
```


```cpp
unsigned int fullPizza = (1 << 20)-1;    // 0000 0000 0000 1111 1111 1111 1111 1111
```
...
```cpp
unsigned int unionSet = a | b;      // 합집합

unsigned int intersection = a & b;  // 교집합

unsigned int removed = a & ~b;      // 차집합

unsigned int xor = a ^ b;           // 합집합에서 교집합을 제외한 집합
```
...
```cpp
int howManyToppings(unsigned int s) {
  int sz= 0;
  while (s) {
    sz += (s&1);
    s >>= 1;
...
```cpp
int howManyToppings2(unsigned int s) {
  if (s == 0) return 0;
  return (s & 1) + setSize(s >> 1);
}
```
...
```cpp
unsigned int firstTopping= pizza & -pizza;
...
```cpp
unsigned int removed = pizza & (pizza-1);
...
to be continued ... 아 졸리다
<br/><br/><br/><br/><br/>



<br/><br/>
<b>Ref.</b> <cite>『알고리즘 문제해결 전략』, 구종만, 인사이트, 2012</cite>